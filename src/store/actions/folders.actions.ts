...
export function initFolders(folderType:string){
    return (dispatch:any) => {
        HttpProvider.get(`/folders/${folderType}`)
        .then(folders => {
            dispatch({
                type : folderType === 'template' ? INIT_TEMPLATE_FOLDERS : INIT_DATASHEET_FOLDERS,
                payload : folders
            })
        })
    }
...
export function toogleFolder(folderType:string, folder:IFolder):IAction{
    ...
    return {
        type : folderType === 'template' ? TOOGLE_TEMPLATE_FOLDER : TOOGLE_DATASHEET_FOLDER,
        payload : folder
    }
}
...
export function createFolder(folderType:string, name:string){
    return (dispatch: any, getState: any) => {
        const foldersTree:IFolderTree = getState()[`${folderType}Folders`].foldersTree;
