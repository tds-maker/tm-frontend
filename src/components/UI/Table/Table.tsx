...
export default class Table extends React.PureComponent<IProps>{
    ...
    public render(){
        ...
        return (<table className={hasScroll ? "tm-table fixed" : "tm-table"}>
            <thead className={hasScroll ? "has-scroll" : ""}>
                <tr>
                    {columns.map((column, key) => {
                        return <th key={key} style={column.style} className={column.className}>{column.header}</th>
                    })}
                </tr>
            </thead>
    ...
    private renderTD(item:any, key:number, column:IColumn){
        return <td className={column.className} style={column.style} key={key}>{item}</td>;
    }
}