...
class SelectList extends React.PureComponent<IProps, IState> {
  ...
  public componentWillReceiveProps(nextProps: IProps) {
    this.setState({
      selectedItem: nextProps.items.find(
        x => x.value === nextProps.selectedValue
      )
    });
  }
  ...
  public render() {
    ...
    return (
      ...
      >
        ...
        {isOpen ? (
          <ul className={itemsClassnames}>{items.map(this.renderItem)}</ul>
        ) : null}
      </div>
    );
  }
  ...
  private renderItem(item: ISelectItem, index: number) {
    const isSelected =
      this.state.selectedItem && this.state.selectedItem.value === item.value;
